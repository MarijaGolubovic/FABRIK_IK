#include <fabrik_ik/fk_moveit.hpp>
#include <fabrik_ik/goal.hpp>
#include <fabrik_ik/ik_gradient.hpp>
#include <fabrik_ik/ik_memetic.hpp>
#include <fabrik_ik/fabrik_ik_plugin.hpp>

#include <fabrik_ik/fabrik_ik_parameters.hpp>
...
bool FabrikIKPlugin::searchPositionIK(std::vector<geometry_msgs::msg::Pose> const& ik_poses,
...
std::vector<std::string> const& FabrikIKPlugin::getJointNames() const { return joint_names_; }

std::vector<std::string> const& FabrikIKPlugin::getLinkNames() const { return link_names_; }

bool FabrikIKPlugin::getPositionFK(std::vector<std::string> const&,
                                 std::vector<double> const&,
                                 std::vector<geometry_msgs::msg::Pose>&) const {
    return false;
}

bool FabrikIKPlugin::getPositionIK(geometry_msgs::msg::Pose const&,
                                 std::vector<double> const&,
                                 std::vector<double>&,
...
bool FabrikIKPlugin::searchPositionIK(geometry_msgs::msg::Pose const& ik_pose,
                                    std::vector<double> const& ik_seed_state,
                                    double timeout,
                                    std::vector<double>& solution,
                                    moveit_msgs::msg::MoveItErrorCodes& error_code,
...
PLUGINLIB_EXPORT_CLASS(fabrik_ik::FabrikIKPlugin, kinematics::KinematicsBase);
